...
class instagram_praser:
    def __init__(self,user):
        self.root='https://www.instagram.com/'
        self.url=self.root+user
        ...
        self.html=self.open_html(self.url)
    ...
    def get_urls(self):
        urls = []
        html_decode = self.html.decode('utf-8')
        ...
        for item in items:
            if item.text().strip().startswith('window._sharedData'):
                ...
                # print(cursor, flag)
        while flag:
            url =self.uri.format(user_id=user_id, cursor=cursor)
            # print(url)
